...
def main():
    ...
    print(args.tgt)
    parts = args.tgt.split('/')
    wav_scp = '/'.join(parts[:-1]) + '/wav.scp'
    extra_wav = '/'.join(parts[:-1]) + '/extra.tsv'
    with open(wav_scp, 'r') as r1, open(extra_wav, 'w') as w1:
        for line in r1.readlines():
            line = line.strip().split(' ')
            uttid, wav_path = line[0], line[1]
            w1.writelines(wav_path + ' ' + str(sf.info(wav_path).frames) + '\n')
            # w1.writelines(uttid + '\n')
            

    extra = '/'.join(parts[:-1]) + '/extra.phn'
    # if 'transcript' not in args.tgt:
    if 1:
        with open(args.src, 'r') as rf, open(args.tgt, 'w') as wf, open(extra, 'w') as w1:
            for line in rf.readlines():
                line = line.strip().split(' ')
                uttid, utt = line[0], line[1:]
                map_utt = [ map_dict[phone] for phone in utt if map_dict[phone] != "" ]
                wf.writelines(uttid + ' ' + ' '.join(map_utt) + '\n')
                w1.writelines(' '.join(map_utt) + '\n')
                # w1.writelines(uttid + '.WAV\n')
        return
